...
template <typename Dest, typename Src, typename Gamma>
inline mizuiro::color::object<Dest> same_to_same_gamma(
    Src const &_src,
    Gamma const _gamma,
    typename mizuiro::color::object<Dest>::format_store_type const &_format)
{
  return mizuiro::color::conversion::detail::convert_each<Dest>(
      _src,
      [_gamma](auto const &_channel_inner, auto const &_src_inner, auto &_dest_inner)
      {
        _dest_inner.set(
            _channel_inner,
            mizuiro::color::denormalize<mizuiro::color::format::get<MIZUIRO_DECLTYPE(_dest_inner)>>(
                _dest_inner.format_store(),
                _channel_inner,
                std::pow(
                    mizuiro::color::normalize<
                        // TODO(philipp)!
                        float>(_src_inner, _channel_inner),
                    _gamma)));
      },
      _format);
}
