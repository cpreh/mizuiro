...
template <typename Color1, typename Color2, typename CompareChannel>
bool compare(Color1 const &_color1, Color2 const &_color2, CompareChannel const &_compare)
{
  static_assert(
      mizuiro::color::format::compatible<
          mizuiro::color::format::get<Color1>,
          mizuiro::color::format::get<Color2>>::value,
  ...
  return fcppt::algorithm::all_of(
      mizuiro::color::access::channels<typename Color1::format>(_color1.format_store()),
      [&_color1, &_color2, &_compare](auto const _channel) -> bool
      {
        return _compare(
            mizuiro::color::decay_channel_proxy(_color1.get(mizuiro::color::get_channel(_channel))),
            mizuiro::color::decay_channel_proxy(
                _color2.get(mizuiro::color::get_channel(_channel))));
      });
}
