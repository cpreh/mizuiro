...
mizuiro::image::raw_container<T, A>::operator=(
	raw_container const &other
)
{
	destroy();
...
>
void
mizuiro::image::raw_container<T, A>::resize(
	size_type const nsz
)
{
	destroy();
...
mizuiro::image::raw_container<T, A>::data()
{
	return data_;
}
...
>
void
mizuiro::image::raw_container<T, A>::allocate(
	size_type const sz
)
{
...
>
void
mizuiro::image::raw_container<T, A>::copy(
	raw_container const &other
)
{
	allocate(
...
>
void
mizuiro::image::raw_container<T, A>::destroy()
{
	if(data_)
		allocator.destroy(
			data_
