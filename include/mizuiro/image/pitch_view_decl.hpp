...
class pitch_view final : private mizuiro::image::format::base<Format>
{
...
public:
  ...
  explicit pitch_view(mizuiro::image::pitch_view<access, format, OtherConstness> const &);

  [[nodiscard]] dim const &size() const;

  [[nodiscard]] pitch_type const &pitch() const;
