...
inline void copy_same_channel_order(
    ViewS const &_src,
    ViewD const &_dest,
    mizuiro::image::algorithm::may_overlap,
    mizuiro::image::algorithm::uninitialized const
        _uninitialized // NOLINT(readability-avoid-const-params-in-decls)
)
{
...
template <
    ...
    typename Constness2>
inline void copy_same_channel_order(
    mizuiro::image::linear_view<Access1, Format1, Constness1> const &_src,
    mizuiro::image::linear_view<Access2, Format2, Constness2> const &_dest,
    ...
    mizuiro::image::algorithm::uninitialized)
{
  ...
  case mizuiro::image::algorithm::may_overlap::yes:
    mizuiro::detail::copy_overlap(_src.begin().data(), _src.end().data(), _dest.begin().data());
    return;
