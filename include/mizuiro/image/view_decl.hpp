...
public:
  ...
  using format_store_type = mizuiro::image::format::store<format>;
  ...
  using linear_view = mizuiro::image::linear_view<access, format, constness>;

  using pitch_view = mizuiro::image::pitch_view<access, format, constness>;

  using view_variant = fcppt::variant::object<linear_view, pitch_view>;
  ...
  using pitch_type = mizuiro::image::pitch_type<typename format::dim>;

  explicit view(linear_view const &);

  explicit view(pitch_view const &);
  ...
  explicit view(view<access, format, OtherConstness> const &);

  [[nodiscard]] dim size() const;

  [[nodiscard]] pitch_type pitch() const;

  [[nodiscard]] iterator begin() const;

  [[nodiscard]] iterator end() const;

  [[nodiscard]] reference operator[](dim const &) const;

  [[nodiscard]] pointer data() const;

  [[nodiscard]] format_store_type format_store() const;

  [[nodiscard]] view_variant const &impl() const;
