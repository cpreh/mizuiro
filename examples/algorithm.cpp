...
using d2_format = mizuiro::image::format::interleaved<mizuiro::image::dimension<2>, ColorFormat>;
...
try
{
  ...
  using format1 = d2_format<
      mizuiro::color::format::homogenous_static<channel_type, mizuiro::color::layout::rgba>>;

  using format2 =
      d2_format<mizuiro::color::format::homogenous_static<float, mizuiro::color::layout::argb>>;

  using store1 = mizuiro::image::store<format1>;

  using store2 = mizuiro::image::store<format2>;

  store1::dim const dim(4U, 2U);

  store1 img1{
      dim,
      mizuiro::color::object<format1::color_format>(
  ...
  mizuiro::image::algorithm::print(std::cout, img2.view());

  std::cout << "\n\nequal: " << std::boolalpha
            << mizuiro::image::algorithm::equal(img1.view(), img1.view()) << '\n';
  ...
  mizuiro::image::algorithm::transform(
      img2.view(),
      img1.view(),
      [](auto const &_src, auto const &_dest)
      {
        mizuiro::color::for_each_channel(
            _src,
            [&_src, &_dest](auto const &_channel)
            {
              _dest.set(
                  _channel,
                  static_cast<mizuiro::color::types::channel_value<
                      mizuiro::color::format::get<MIZUIRO_DECLTYPE(_dest)>,
                      MIZUIRO_DECLTYPE(_channel)>>(_src.get(_channel)));
            });
      },
